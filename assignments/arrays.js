...
function getCarModel() {
    for (let i = 0; i < inventory.length; i++) {
        const arr = inventory[i];
        carModels.push(arr.car_model);
    }
...
let BMWAndAudi = [];
function findBmwAudi() {
    for (let i = 0; i < inventory.length; i++) {
        const arr = inventory[i];
        let carMake = arr.car_make;
        if (carMake == "BMW" || carMake == "Audi") {
            BMWAndAudi.push(arr);
        }
    }
    let cars = JSON.stringify(BMWAndAudi);
    return cars;
}
console.log(findBmwAudi());

console.log("********* END OF ARRAY.JS *********");